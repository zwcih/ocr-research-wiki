...
已摄入 **81 篇** source，覆盖从深度学习起点到最新端到端 OCR 的完整技术谱系，包括 DETR 检测、场景文字、文档版面分析、识别/解析模型、训练数据与解码加速。
...
**DETR 检测线** [[detr]]→[[deformable-detr]]→[[dino-detr]]（query-based，去 anchor/NMS），文档落地为 [[rt-doclayout]]（RT-DETR 单query出框/mask/阅读顺序）+ [[parser-oriented-refinement]]（稳定检测→解析器界面，NMS-free）；困难场景文字由 [[armorocr]] 将定位、识别、full spotting 与区域 VQA 组合成可验证训练任务，并用 transformed-view teacher 蒸馏原图感知。生成线 [[ddpm]]。
